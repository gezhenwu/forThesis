        ...
        if ch_nb == channel:
            for modules in ch["Modules"]:
                ...
                for chips in modules["Chips"]:
                    ...
                    if scanValue == options.dacVthcMax:
                    ...
                    for pix in alti.getPixelList(options.pixelInj):
                        ...
                        address = baseaddr + 4                
                        alti.wr_asic_reg(address, scanValue)
        ...
        if ch_nb == channel:
            ...
            for modules in ch["Modules"]:
                ...
                for chips in modules["Chips"]:
                    ...
                    finaltxt = outputbasedir +"/t"+str(ch_nb)+"M"+str(module_nb + 14*ch_nb)+"/vthcScan"+options.suffix+"/"+Utils.getOutName(options)
                    if not os.path.isdir(finaltxt):
                        os.makedirs(finaltxt)
                    prefix = finaltxt            
                    tmptxt = prefix+'txt_data_'+'dacVthc_' + str(scanValue) + '.txt'
                    suffix='dacVthc_' + str(scanValue) + '_.csv'
...
def main():
    ...
    for col in range(options.startCol,options.endCol+1,options.stepCol):
        ...
        for dacVthc in dacVthcList:
            for channel in [1,2]:
                try:
                    scanCfg(channel,col,dacVthc)
                except:
                    print('channel %d col %d dacVthc %d configuration failed'%(channel,col,dacVthc))
                    sys.exit()
            # serialcomm.clear()
            ...
            row_nb_allCH = 0
            for channel in [1,2]:
                row_nb_ch = dataExtract(channel,col,dacVthc,tmpdat)
