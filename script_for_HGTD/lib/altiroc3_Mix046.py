...
class Altiroc(object):
    def __init__(self, devnr, gbt, addr, use_usb, module_id, chip_id, use_fice=False, use_serialcomm=True):
        self.module = peb_Mix046.peb(devnr = devnr, gbt = gbt, addr = addr, use_usb = use_usb, use_fice  = use_fice, use_serialcomm = use_serialcomm)
    ...
    def turnOffCol(self, column):#discri off for columns with no injected signal
        if column == "all":
            for pixel in range(nbOfPixels):
                baseaddr = (16*pixel)+(16*(pixel//15))
                self.wr_asic_reg(baseaddr + 3, 0b00000000)
        else:
    ...
    def turnOnCol(self, column):#discri off for pixels with no injected signal
        pixelInjList = self.getPixelList(column)
        for pixel in pixelInjList:
            baseaddr = (16*pixel)+(16*(pixel//15))

            # For config: no Ctest            
            # self.wr_asic_reg(baseaddr + 3, 0b01111100)
            # For scan: enable Ctest            
            self.wr_asic_reg(baseaddr + 3, 0b01111110)
...
=======
    def tune_PS(self):
        for i in range(8,12):
            self.wr_asic_reg(0x101D, i)
            print("reg 0x101D: ", self.rd_asic_reg(0x101D, read_len=1)[0])
            self.wr_asic_reg(0x101D, 120)
            print("reg 0x101D: ", self.rd_asic_reg(0x101D, read_len=1)[0])
            print("reg 0x2000: ", self.rd_asic_reg(0x2000, read_len=1)[0])
            if self.rd_asic_reg(0x2000, read_len=1)[0] == 127: break
        print("final PS: ", self.rd_asic_reg(0x2000, read_len=1)[0])
