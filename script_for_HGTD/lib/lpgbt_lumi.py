...
class lpgbt(object):
    def __init__(self, serialcomm = None, devnr = 0, gbt = 0, addr = 0x71,  use_fice = False):
        devnr &= 0xFF
        gbt &= 0xFF
        addr &= 0xFF
        self.devnr = devnr
    ...
    def write_byte_ic(self, reg_addr, reg_val, retry = True, debug = False):
        reg_addr &= 0x1FF
        reg_val &= 0xFF
        cmd = "fice -1 -d %d -e -G %d -I 0x%x -a 0x%x 0x%x"%(self.devnr, self.gbt, self.addr, reg_addr, reg_val)
        # print(cmd)
        for i in range(self.retry_max):
            flx = os.popen(cmd)
            reply = flx.read()
    ...
    def read_byte_ic(self, reg_addr, retry = True, debug = False):
        reg_addr &= 0x1FF
        cmd = "fice -1 -d %d -e -G %d -I 0x%x -a 0x%x"%(self.devnr, self.gbt, self.addr, reg_addr)
        # print(cmd)
        for i in range(self.retry_max):
            flx = os.popen(cmd)
            reply = flx.read()
