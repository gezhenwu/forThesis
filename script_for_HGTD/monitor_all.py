...
def main():
    ...
    for in_adc in range(1,16):
        ...
        # print("Timing ADC input "+str(in_adc)+": %d"%peb.timing.lpgbt.adc_convert(1))
        adc_value= peb.timing.lpgbt.adc_convert(1)
        calibrated_v= peb.timing.lpgbt.adc_get_vin(1)
        print("Timing ADC input "+str(in_adc)+": "+ str(adc_value) + " voltage: " + str(calibrated_v*1000))
        time.sleep(0.1)
    # if options.gbt in range(0,3): 
    #     for in_adc in range(1,16):
    #         peb.lumi.lpgbt.adc_config(inp=in_adc, inn=15, gain=0)
    #         print("Lumi ADC input "+str(in_adc)+": %d"%peb.timing.lpgbt.adc_convert(1))
    #         time.sleep(0.1)
    ###########################################
    ...
    for module in [0,2,4,6,8,9,10,11,12,13]:
        for val in [0,1,2,5]:
            ...
            adc_value= peb.timing.lpgbt.adc_convert(1)
            calibrated_v= peb.timing.lpgbt.adc_get_vin(1)
            print("channel %d module %d mux channel %d: ADC: %d, voltage: %d"%(options.gbt,module,mux_ch,adc_value,calibrated_v*1000))
            time.sleep(0.1)
        i+=1
    ############################################################
    ...
    adc_value= peb.timing.lpgbt.adc_convert(1)
    calibrated_v= peb.timing.lpgbt.adc_get_vin(1)
    print("Read TEMP from PEB: "+str(adc_value) + " voltage: " + str(calibrated_v*1000))
    time.sleep(0.1)
