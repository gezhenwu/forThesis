...
def main():
    ...
    for module in [0,2,4,6,8,9,10,11,12,13]:
        for val in [0,1,2,5]:
            ...
            calibrated_v= peb.timing.lpgbt.adc_get_vin(1)
            if val == 0:
                print("channel %d module %d mux channel %d, GND: ADC: %d, voltage: %d"%(options.gbt,module,mux_ch,adc_value,calibrated_v*1000)) 
            if val == 1:
                print("channel %d module %d mux channel %d, VDDA: ADC: %d, voltage: %d"%(options.gbt,module,mux_ch,adc_value,calibrated_v*1000))
            if val == 2:
                print("channel %d module %d mux channel %d, VDDD: ADC: %d, voltage: %d"%(options.gbt,module,mux_ch,adc_value,calibrated_v*1000))
            if val == 5:
            ...
            for chip in [0,1]:
                ...
                try:
                    ...
                    for reg_value in range(96,128):
                        ...
                        calibrated_v= peb.timing.lpgbt.adc_get_vin(1)
                        if k in range(0,12):
                            print("chanel %d module %d chip %d name %s monitor_sig %d: ADC: %d, voltage: %d"%(options.gbt,module,chip,convention["name"],k,adc_value,calibrated_v*1000))
                        if k == 15:
                            print("chanel %d module %d chip %d name %s monitor_sig %d, PS_biasn: ADC: %d, voltage: %d"%(options.gbt,module,chip,convention["name"],k,adc_value,calibrated_v*1000))
                            if calibrated_v*1000 > 450 and calibrated_v*1000 < 550: print("PS_biasn OK")
                            else: print("!!!!!!!!!!!!!!!PS_biasn ERROR!!!!!!!!!!!!")
                        if k == 17:
                        ...
                        if k == 21:
                            ...
                            adc_value= peb.timing.lpgbt.adc_convert(1)
                            calibrated_v= peb.timing.lpgbt.adc_get_vin(1)
                            print("chanel %d module %d chip %d name %s monitor_sig %d, Probe_dc_pa of row8 col7: ADC: %d, voltage: %d"%(options.gbt,module,chip,convention["name"],k,adc_value,calibrated_v*1000))
                            if (calibrated_v*1000 > 600 and calibrated_v*1000 < 700): print("Probe_dc_pa ok")
                            else: print("!!!!!!!!!!!!!!!!Probe_dc_pa ERROR!!!!!!!!!!!!!!!!!")
                        if k == 22:
                            peb.module_i2c_write(
                                module_id = module,
                                chip_id = chip,
                            ...
                            adc_value= peb.timing.lpgbt.adc_convert(1)
                            calibrated_v= peb.timing.lpgbt.adc_get_vin(1)
                            print("chanel %d module %d chip %d name %s monitor_sig %d, Probe_vthc of row8 col7: ADC: %d, voltage: %d"%(options.gbt,module,chip,convention["name"],k,adc_value,calibrated_v*1000))
                        if k ==27:
                            print("chanel %d module %d chip %d name %s monitor_sig %d, vbg: ADC: %d, voltage: %d"%(options.gbt,module,chip,convention["name"],k,adc_value,calibrated_v*1000))
                            if calibrated_v*1000 > 900 and calibrated_v*1000 < 1050: print("vbg OK")
                            else: print("!!!!!!!!!!!!!vbg ERROR!!!!!!!!!!")
                        if k == 28:
                            print("chanel %d module %d chip %d name %s monitor_sig %d, vth: ADC: %d, voltage: %d"%(options.gbt,module,chip,convention["name"],k,adc_value,calibrated_v*1000))
                        if k == 29:
                            calibrated_vc120 = calibrated_v
                            print("chanel %d module %d chip %d name %s monitor_sig %d, Vcontrol 120p: ADC: %d, voltage: %d"%(options.gbt,module,chip,convention["name"],k,adc_value,calibrated_v*1000))
                        if k == 30:
                            print("chanel %d module %d chip %d name %s monitor_sig %d, Vcontrol 140p: ADC: %d, voltage: %d"%(options.gbt,module,chip,convention["name"],k,adc_value,calibrated_v*1000))
                            if (calibrated_v - calibrated_vc120)*1000 > 50 and (calibrated_v - calibrated_vc120)*1000 < 90:
                                print((calibrated_v - calibrated_vc120)*1000, "Vcontrol OK")
                            else:
                                print((calibrated_v - calibrated_vc120)*1000, "!!!!!!!!!!!!Vcontrol ERROR!!!!!!!!!!!!!")
                        if k == 31:
                            print("chanel %d module %d chip %d name %s monitor_sig %d, Vcontrol 160P: ADC: %d, voltage: %d"%(options.gbt,module,chip,convention["name"],k,adc_value,calibrated_v*1000))
                        time.sleep(0.1)
                        k+=1
                except:  
                    continue        
