    ...
    for dacVth in dacVthList:        
        for modules in alti_dict["Modules"]:
            ...
            for chips in modules["chips"]:
                ...
                alti.setDacVth(dacVth, options.dacVthRange)
        cnt+=1
        alti.byebye()
        ...
        my_reg_addr=0x1000
        my_reg=alti.rd_asic_reg(my_reg_addr,1)
        print("-----------------------------------------------------ALTIROC3 reg_reading-------------------------------------")
        print("reg_addr = " , my_reg_addr)
        print("value = ", my_reg)
        ...
        for modules in alti_dict["Modules"]:
            ...
            for chips in modules["chips"]:
                chip_nb = chips["chip"]
                alti = altiroc3.Altiroc(devnr = devnr, gbt = gbt, addr = timing_addr, use_usb = False, module_id = module_nb, chip_id = chip_nb)
                finaltxt = outputbasedir +"/M"+str(module_nb + 14*gbt)+"/thresScan"+options.suffix+"/"+Utils.getOutName(options)
                if not os.path.isdir(finaltxt):
                    os.makedirs(finaltxt)
                prefix = finaltxt            
                tmptxt = prefix+'txt_data_'+'dacVth_' + str(dacVth) + '.txt'                
                suffix='dacVth_' + str(dacVth) + '_.csv'
