        ...
        if ch_nb == channel:
            for modules in ch["Modules"]:
                ...
                for chips in modules["Chips"]:
                    ...
                    if scanValue == options.dacChargeMax:
        ...
        if ch_nb == channel:
            ...
            for modules in ch["Modules"]:
                ...
                for chips in modules["Chips"]:
                    ...
                    finaltxt = outputbasedir +"/t"+str(ch_nb)+"M"+str(module_nb + 14*ch_nb)+"C"+str(chip_nb)+"/chargeScan"+options.suffix+"/"+Utils.getOutName(options)
                    if not os.path.isdir(finaltxt):
                        os.makedirs(finaltxt)
                    prefix = finaltxt            
                    tmptxt = prefix+'txt_data_'+'dacCharge_' + str(scanValue) + '.txt'
                    suffix='dacCharge_' + str(scanValue) + '_.csv'
...
def main():
    ...
    for col in range(options.startCol,options.endCol+1,options.stepCol):
        ...
        for dacCharge in dacChargeList:
            for channel in [1,2]:
                try:
                    scanCfg(channel,col,dacCharge)
                except:
                    print('channel %d col %d dacCharge %d configuration failed'%(channel,col,dacCharge))
                    sys.exit()
            ...
            row_nb_allCH = 0
            for channel in [1,2]:
                row_nb_ch = dataExtract(channel,col,dacCharge,tmpdat)
