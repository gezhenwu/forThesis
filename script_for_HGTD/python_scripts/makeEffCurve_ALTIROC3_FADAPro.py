...
def make_curves_with_scanpoints(df, scanpoints, scanindexedby='pixelInj', scanvar='dac', nmax=20, ascending=True):
    ...
    for ip in inj_patterns:
        ...
        for pix in exp_pixels:
            if pix in eff_curves: # one pixel shoudl be checked only once
...
def make_vthc_file(foutname, thr_dict, vals_default=128):
    ...
    for ipix in range(225):
        ...
        baseaddr = (16*ipix)+(16*(ipix//15))
        f.write(lproto.format(addr=baseaddr+4, val=val))
    f.close()
...
def get_pixel_list(pixStr):
    if 'col' in pixStr:
        ...
        return [15*icol + irow for icol in range(15)]
    else:
        return [int(pixStr.replace('pix', ''))]
...
if __name__ == '__main__':
    ...
    if infos['measType'] == 'thresScan':
        scanpoints = list(range(0, 1024))  ## increasing Vth -> reducing eff
        timing_fname = 'timing_data_dacVth_{dac}.csv'
        ascending=True
    elif infos['measType'] == 'vthcScan':
        scanpoints = list(reversed(range(0, 256))) ## reducing Vth -> reducing eff
        timing_fname = 'timing_data_dacVthc_{dac}.csv'
        ascending=False
    elif infos['measType'] == 'chargeScan':
        scanpoints = list(reversed(range(0, 60))) #128# reducing charge -> reducing eff
        timing_fname = 'timing_data_dacCharge_{dac}.csv'
        ascending=False
    else:
        raise RuntimeError("measurement type %s not understood, cannot guess scanpoints" % infos['measType'])
    ...
    for iasic in range(nasic):
        ...
        for pix in pixlist:
            if args.doOnly and pix not in toplot:
                continue
            axs_crv[iasic].plot(ec[iasic][pix]['x'], ec[iasic][pix]['y'], plotstyle, label=str(pix))
