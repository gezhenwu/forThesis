...
# width can only be done with ext_disc
options.ext_discri=True
...
print('elinkConfigFile',elinkConfigFile)
...
for col in range(options.startCol,options.endCol+1,options.stepCol):
    ...
    widthList = list(range(options.widthMin,options.widthMax+1,options.widthStep))
    ...
    for width in widthList:
        for modules in alti_dict["Modules"]:
            ...
            for chips in modules["chips"]:
                ...
                alti.wr_asic_reg(0x2007, width)
        cnt+=1
        alti.byebye()    

        alti.cmdtrigger(options.triggerFre)  
        alti.cmdgbrst()
        tmpdat = outputbasedir+'/Rawdat_width/col_'+str(col)+'_width'+ str(width)
        ...
        for modules in alti_dict["Modules"]:
            ...
            for chips in modules["chips"]:
                chip_nb = chips["chip"]
                alti = altiroc3.Altiroc(devnr = devnr, gbt = gbt, addr = timing_addr, use_usb = False, module_id = module_nb, chip_id = chip_nb)
                finaltxt = outputbasedir +"/M"+str(module_nb + 14*gbt)+"C"+str(chip_nb)+"/widthScan"+options.suffix+"/"+Utils.getOutName(options)
                if not os.path.isdir(finaltxt):
                    os.makedirs(finaltxt)
                prefix = finaltxt            
                tmptxt = prefix+'txt_data_'+'width'+ str(width)+ '.txt'
                # os.system('rm'+tmptxt)
                suffix='width_'+ str(width)+ '_.csv'
        ...
        if options.autoStop:
            ...
            if len(data_size_all) > 5 and Utils.is_all_zero(data_size_all[-6])==False:
                if Utils.is_all_zero(data_size_all[-5]) & Utils.is_all_zero(data_size_all[-4]) & Utils.is_all_zero(data_size_all[-3]) & Utils.is_all_zero(data_size_all[-2]) & Utils.is_all_zero(data_size_all[-1]):
                    print("Stop scan at width = %d for column %d"%(width,col))
                    break  
