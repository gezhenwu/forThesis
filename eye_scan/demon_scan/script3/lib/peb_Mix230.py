...
class peb(object):
    ...
    def module_reset(self, module_id):
        if (module_id<0) or (module_id>13):
            ...
            return
        elif module_id in [0,4]:
            temp = BIT_RST_0
            text = "0, 4"
        elif module_id in [8,10,12]:
            temp = BIT_RST_1
            text = "8, 10, 12" 
        else:
            print("Module %d does not in this pattern."%module_id)            

        if module_id in [0,4,8,10,12]:            
    ...
    def module_power(self, module_id, enable):
        ...
        if enable:
            ...
            if module_id in [0,4]:
                temp = BIT_PEN_0
                text = "0, 4"                
                if self.ep_config[0].enable or self.ep_config[4].enable:
                    print("Modules sharing same power group must be enabled or disabled simultaneously.")
                    return                
            elif module_id in [8,10,12]:
                temp = BIT_PEN_1
                text = "8, 10, 12"                
                if self.ep_config[8].enable or self.ep_config[10].enable or self.ep_config[12].enable:
                    print("Modules sharing same power group must be enabled or disabled simultaneously.")
                    return
            else:
                print("Module %d does not in this pattern."%module_id)
